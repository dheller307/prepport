...
export function PrepSessionDetail({ id, onBack }: PrepSessionDetailProps) {
  ...
  const [loadingErrorIngredients, setLoadingErrorIngredients] = useState<
    string | null
  >(null);
  const [loadingErrorSession, setLoadingErrorSession] = useState<string | null>(
    null,
  );
  const [isLoadingSession, setIsLoadingSession] = useState<boolean>(false);
  const [isLoadingIngredients, setIsLoadingIngredients] =
    useState<boolean>(false);
  const [loadingErrorPortionLogs, setLoadingErrorPortionLogs] = useState<
    string | null
  >(null);
  const [isLoadingPortionLogs, setIsLoadingPortionLogs] =
    useState<boolean>(false);
  ...
  useEffect(() => {
    async function loadPrepSession() {
      ...
      try {
        ...
        setLoadingErrorSession(
          error instanceof Error
            ? error.message
            : "Failed to load prep session",
        );
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = isEditing
        ? await updateBatch(id, editingBatch.id!, form)
        : await createBatch(id, form);
      if (isEditing) {
        setPrepSession((current) =>
          current
            ? {
                ...current,
                batches: (current.batches ?? []).map((batch) =>
                  batch.id === editingBatch.id ? response : batch,
                ),
              }
            : current,
        );
        ...
        setPrepSession((current) =>
          current
            ? { ...current, batches: [...(current.batches ?? []), response] }
            : current,
        );
      }
      setForm(emptyBatchForm);
            ...
            <form className="entity-form" onSubmit={handleSubmit}>
              ...
              <div className="form-field">
                ...
                <select
                  ...
                  disabled={isLoadingIngredients || !!loadingErrorIngredients}
                  onChange={(e) =>
                    setForm({ ...form, ingredientId: Number(e.target.value) })
                  }
                >
              ...
              <div className="form-field">
                ...
                <input
                  ...
                  id="rawWeightG"
                  placeholder="0"
                  value={form.rawWeightG === 0 ? "" : form.rawWeightG}
                  onChange={(e) =>
                    setForm({ ...form, rawWeightG: Number(e.target.value) })
                  }
                />
              </div>
              <div className="form-field">
                ...
                <input
                  ...
                  id="cookedWeightG"
                  placeholder="0"
                  value={form.cookedWeightG === 0 ? "" : form.cookedWeightG}
                  onChange={(e) =>
                    setForm({ ...form, cookedWeightG: Number(e.target.value) })
                  }
                />
              </div>
              <p className="hint">
                Both weights are required. If nothing changed in the pan, enter
            ...
            <ul className="batch-list">
              {prepSession.batches?.map((batch) => (
                <li key={batch.id} className="batch-card">
                  ...
                  <section
                    className="batch-inventory"
                    aria-label="Batch inventory"
                  >
                    <dl className="batch-stats">
                      ...
                      <div>
                        <dt>Available</dt>
                        <dd>
                          {calculateAvailableCookedGrams(batch).toFixed(1)} g
                        </dd>
                      </div>
                    </dl>
                  </section>
                    ...
                    <details className="batch-history">
                      <summary>
                        <span className="batch-history-title">
                          <span
                            className="batch-history-indicator"
                            aria-hidden="true"
                          >
                            ▸
                          </span>
                          ...
                          <ul>
                            {portionLogs.flatMap((log) =>
                              log.lines
                                ...
                                .map((line) => (
                                  <li key={log.id}>
                                    {log.portionDate} · {log.name} ·{" "}
                                    {line.cookedGrams} g used
                                  </li>
                                )),
                            )}
                          </ul>
