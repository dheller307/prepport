...
import { PortionLogResponse, PortionLogRequest } from '../types/portionLog';
import { calculatePortion, exportPortion } from '../api/portion';
import { listPortionLogs, updatePortionLog, deletePortionLog, createPortionLog } from '../api/portionLogs';
...
export function PortionBuilder() {
    ...
    const [editingPortionLogId, setEditingPortionLogId] = useState<number | null>(null);
    ...
    useEffect(() => {
        async function loadPortionLogs() {
            ...
            try {
                ...
                setPortionLogError(portionLogLoadError instanceof Error ? portionLogLoadError.message : 'Failed to load meals');
            } finally {
    ...
    function calculateAvailableCookedGrams(batchId: number) {
        ...
        const usedCookedGrams = portionLogs.filter((log => log.id !== editingPortionLogId)).flatMap((log) => log.lines).filter((line) => line.batchId === batchId).reduce((total, line) => total + line.cookedGrams, 0);
    ...
    async function calculateLine(index: number) {
        ...
        try {
            ...
            setCalculateError(calculateError instanceof Error ? calculateError.message : 'Failed to calculate meal ingredient');
        } finally {
    ...
    async function handleExport() {
        ...
        try {
            ...
            setExportError(exportError instanceof Error ? exportError.message : 'Failed to generate nutrition-tracker text');
            setExportText(null);
        } finally {
    ...
    async function handleSavePortionLog() {
        ...
        if (validLines.length === 0 || validLines.length !== lines.length) {
            setPortionLogError('All lines must have a batch and positive cooked grams');
            return;
        }
        ...
        try {
            const response = editingPortionLogId !== null ? await updatePortionLog(editingPortionLogId, request) : await createPortionLog(request);
            setPortionLogs(prev => [response, ...prev.filter((log) => log.id !== response.id)]);
            clearPortionLogForm();
        } catch (portionLogSaveError) {
            setPortionLogError(portionLogSaveError instanceof Error ? portionLogSaveError.message : 'Failed to save meal');
        } finally {
    ...
    function handleEditPortionLog(portionLog: PortionLogResponse) {
        setLines(portionLog.lines.map((line) => ({ batchId: line.batchId, cookedGrams: line.cookedGrams })));
    ...
    async function handleDeletePortionLog(id: number) {
        ...
        try {
            ...
            setPortionLogError(portionLogDeleteError instanceof Error ? portionLogDeleteError.message : 'Failed to delete meal');
        } finally {
    ...
    return (
        <div>
            ...
            <p className="page-lede">
                Combine cooked amounts from your prep batches into one meal, then save it or copy its nutrition-tracker text.
            </p>
            {prepSessionLoadingError && <p>{prepSessionLoadingError}</p>}
            {prepSessionIsLoading && <p>Loading prep sessions...</p>}
            {portionLogError && <p>{portionLogError}</p>}
            ...
            <>
                <section className="meal-builder-card">
                    ...
                    <button type="button" onClick={handleAddLine}>Add ingredient</button>
                    {lines.length < 1 && (
                                ...
                                <li key={index} className='portion-line'>
                                    <label>
                                        ...
                                        <select value={line.batchId || ''} onChange={(e) => updateLineBatch(index, Number(e.target.value))}>
                                            ...
                                            {batches.map((batch) => (
                                                <option key={batch.id} value={batch.id}>{batch.ingredient.name} - {batch.createdAt?.split('T')[0]} - {formatAmount(batch.rawWeightG)} g raw / {formatAmount(batch.cookedWeightG)} g cooked</option>
                                            ))}
                                        </select>
                                    </label>
                                    <label>
                                    ...
                                    {line.batchId > 0 && (
                                        <p className="hint">
                                            {formatAmount(calculateAvailableCookedGrams(line.batchId))} g available
                                        </p>
                                    )}
                                    <div className='portion-line-actions'>
                                        <button type="button" onClick={() => calculateLine(index)} disabled={isCalculating}>
                                            {isCalculating ? 'Calculating...' : 'Calculate nutrition'}
                                        </button>
                                        <button type="button" onClick={() => handleRemoveLine(index)}>Remove ingredient</button>
                                    </div>
                                    {results[index] && (
                                        <div className="meal-nutrition-result">
                                            <p>
                                                Nutrition-tracker amount: {formatAmount(results[index].cronometerG)} g
                                            </p>
                                            <p>
                                                Protein: {formatAmount(results[index].proteinG)} g · Fat: {formatAmount(results[index].fatG)} g · Carbs: {formatAmount(results[index].carbsG)} g · Calories: {formatAmount(results[index].kcal)} kcal
                                            </p>
                                        </div>
                                    )}
                                </li>
                    ...
                    <div className="meal-builder-heading">
                        ...
                        <h2>{editingPortionLogId === null ? 'Name this meal' : `Editing: ${portionLogName}`}</h2>
                        <p className="hint">
                            Saving records these cooked amounts and updates each batch&apos;s available amount.
                        </p>
                    </div>
                    ...
                    <div className="meal-builder-heading">
                        ...
                        <p className="hint">
                            Save the meal to track batch usage. Generating tracker text only prepares copyable text, it does not save the meal or change availability.
                        </p>
                    </div>
                    {hasCompleteNutrition ? (
                        <div className="meal-aggregate-nutrition">
                            ...
                            <p>
                                Protein: {formatAmount(currentMealTotals.proteinG)} g · Fat: {formatAmount(currentMealTotals.fatG)} g · Carbs: {formatAmount(currentMealTotals.carbsG)} g · Calories: {formatAmount(currentMealTotals.kcal)} kcal
                            </p>
                        </div>
                    ) : (
                        <p className="hint">
                    ...
                    <div className="meal-finish-actions">
                        <button type="button" onClick={handleSavePortionLog} disabled={isSavingPortionLog}>
                            {isSavingPortionLog
                                ? 'Saving...'
                        ...
                        <button type="button" onClick={handleExport} disabled={isExporting || validLines.length === 0}>
                            {isExporting ? 'Generating...' : 'Generate nutrition-tracker text'}
                        </button>
                        {editingPortionLogId !== null && (
                    ...
                    {exportText && (
                        <div className="meal-export-result">
                            ...
                            <button type="button" onClick={handleCopy} disabled={!exportText}>
                ...
                <section className="recent-meals">
                    <h2>Recent meals</h2>
                    <p className="hint">Edit or delete a meal to update its recorded batch usage.</p>
                                ...
                                <li key={portionLog.id} className="card">
                                    ...
                                    <ul>
                                        {portionLog.lines.map((line) => (
                                            <li key={line.batchId}>
                                                {line.ingredientName}: {formatAmount(line.cookedGrams)} g
                                            </li>
                                        ))}
                                    </ul>
                                    <div className="meal-card-totals">
                                        <p>
                                            Protein: {formatAmount(portionLog.totalProteinG)} g · Fat: {formatAmount(portionLog.totalFatG)} g · Carbs: {formatAmount(portionLog.totalCarbsG)} g · Calories: {formatAmount(portionLog.totalKcal)} kcal
                                        </p>
                                    </div>
                                    <div className="form-actions">
                                        <button type="button" onClick={() => handleEditPortionLog(portionLog)}>
                                            Edit
                                        </button>
                                        <button
                                            type="button"
