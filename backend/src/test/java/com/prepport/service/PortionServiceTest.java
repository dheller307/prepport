...
class PortionServiceTest {
  ...
  void calculatePortion_chickenBatch() {
    when(batchRepository.findByIdAndPrepSession_User(1L, user))
        .thenReturn(Optional.of(chickenBatch()));

    PortionCalculateResponse result =
        portionService.calculatePortion(new PortionCalculateRequest(1L, 200.0), user);
