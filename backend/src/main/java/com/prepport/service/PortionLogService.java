...
public class PortionLogService {
  ...
  public PortionLogResponse getPortionLog(Long id, User user) {
    return toResponse(
        portionLogRepository
            ...
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Portion log not found")));
  }

  @Transactional
  public PortionLogResponse updatePortionLog(Long id, PortionLogRequest request, User user) {
    PortionLog portionLog =
        portionLogRepository
            .findByIdAndUser(id, user)
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Portion log not found"));
  ...
  private List<PortionLogLine> buildValidatedLines(
      PortionLogRequest request, User user, Long excludedPortionLogId) {
    ...
    for (Map.Entry<Long, Double> entry : requestedByBatchId.entrySet()) {
      ...
      Batch batch =
          batchRepository
              .findByIdAndPrepSession_User(batchId, user)
              .orElseThrow(
                  () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Batch not found"));

      double alreadyUsed =
          excludedPortionLogId == null
              ? portionLogLineRepository.sumCookedGramsByBatchId(batchId)
              : portionLogLineRepository.sumCookedGramsByBatchIdExcludingPortionLogId(
                  batchId, excludedPortionLogId);
      if (alreadyUsed + cookedGrams > batch.getCookedWeightG()) {
        throw new ResponseStatusException(
            HttpStatus.BAD_REQUEST,
            batch.getIngredient().getName()
                + " has only "
                + (batch.getCookedWeightG() - alreadyUsed)
                + " grams left in this batch.");
      }
  ...
  private PortionLogResponse toResponse(PortionLog portionLog) {
    ...
    for (PortionLogLine line : portionLog.getLines()) {
      PortionCalculateResponse calculation =
          portionService.calculateForBatch(line.getBatch(), line.getCookedGrams());

      lines.add(
          new PortionLogLineResponse(
              line.getBatch().getId(),
