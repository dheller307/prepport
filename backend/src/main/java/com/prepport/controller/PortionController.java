...
public class PortionController {
  ...
  public PortionCalculateResponse calculatePortion(
      @Valid @RequestBody PortionCalculateRequest request, @AuthenticationPrincipal User user) {
    return portionService.calculatePortion(request, user);
  }

  @PostMapping(value = "/export", produces = MediaType.TEXT_PLAIN_VALUE)
  public String exportPortion(
      @Valid @RequestBody PortionExportRequest request, @AuthenticationPrincipal User user) {
